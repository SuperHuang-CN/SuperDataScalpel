...
本约定适用于 Admin 的 `/api/**` 业务接口，以及 Service Engine 的 `/internal/**` 和 `/open-api/**` 接口。Actuator、Swagger/OpenAPI 等第三方或框架端点不强制改写为该格式。
...
Admin 的认证失败和无权限访问、Service Engine 的管理 Token 过滤和安全拒绝都必须使用公共 Writer。Task Engine 回执 Filter 仅处理 `/api/v1/internal/task-runs/**`；作为 Servlet Filter 注册时，必须显式跳过所有其他路径，不能影响登录或普通业务接口。禁止手工拼装错误 JSON，禁止对 API 使用 `HttpServletResponse.sendError`。
