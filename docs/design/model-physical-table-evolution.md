...
- 计划创建只允许 `MANAGED` 模式且模型为 `DRAFT` 或 `DISABLED`；`PUBLISHED` 必须先停用，`EXTERNAL` 第一版不允许修改。
...
1. **字段定义页签**负责暂存字段编辑。在受管物理表不存在时，仍可直接保存字段；物理表已匹配时，主操作改为“生成变更计划”，不会直接改模型字段。
...
- 物理表已严格匹配时，直接保存请求拒绝，要求通过变更计划执行；
- 物理表已漂移、不可访问或不受支持时，也拒绝直接保存，避免元数据进一步偏离真实表；用户应先修复物理表状态或取消当前操作。

该规则由后端执行，前端只据实时检查结果选择正确按钮。`EXTERNAL` 模型仍沿用原有“保存定义后重新校验/绑定”的流程；本轮不扩展外部表的受控改表能力。
...
字段页签会实时读取物理表状态：`MATCHED` 时把“保存字段”替换成“生成变更计划”；其他非 `NOT_FOUND` 的受管状态显示持续告警并禁用保存，确保用户先处理漂移/连通性问题。计划生成成功后自动打开计划详情抽屉，用户无需在页面间寻找新计划。
...
- UI 静态逻辑测试覆盖：策略/风险/状态标签和“是否允许由字段页直接保存”的判断。
- `pnpm check` 覆盖类型检查、lint、生产构建和前端单元测试。
- 后端集成测试覆盖：物理表已匹配时 `update-fields` 被拒绝；物理表不存在时仍能直接保存，确保不会破坏新模型配置流程。
