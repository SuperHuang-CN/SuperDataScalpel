  ...
  "measurements": [
    {
      ...
      "mode": "SPHEROID",
      "outputColumnName": "road_length_m"
    },
    {
      "kind": "AREA",
      "geometryColumnName": "region_boundary",
      "mode": "SPHEROID",
      "outputColumnName": "region_area_m2"
    },
    {
      "kind": "DISTANCE",
...
结果单位来自 CRS 坐标单位；面积为坐标单位平方。当前平台只保存稳定
`authority + code`，不维护完整 EPSG 单位目录，因此节点不把 PLANAR 结果自动换算为米，
也不在输出 Schema 中虚构单位元数据。

EPSG:4326 使用 PLANAR 时结果是角度或角度平方。计划仍可执行，但每个相关测量项产生
`PLANAR_MEASURE_USES_ANGULAR_UNITS` Warning，Inspector 同步提示优先选择 SPHEROID 或先
显式转换到合适的投影 CRS。

### 3.2 SPHEROID

SPHEROID 固定使用 WGS84 椭球语义：
...
| `PLANAR_MEASURE_USES_ANGULAR_UNITS` | EPSG:4326 使用 PLANAR；级别为 WARNING |
...
- 仅 AREA/LENGTH/PERIMETER/DISTANCE 显示 PLANAR/SPHEROID 选项。
- 根据上游 GeometryKind 过滤候选；已保存但失效的字段继续显示并标红。
- SPHEROID 选中非 4326 字段时原值保留并显示错误；PLANAR + 4326 显示单位警告。
- 面板明确展示单位：SPHEROID 为米/平方米，PLANAR 为 CRS 坐标单位/平方。
...
安全摘要只允许记录表名、参与字段名、输出字段名、测量类型、模式、数量和 CRS。日志、
...
- 六种测量项和两种 mode 可稳定 JSON 往返，未知判别类型被拒绝。
...
- 单位自动换算、自定义地球半径、自定义椭球和 EPSG 单位目录。
