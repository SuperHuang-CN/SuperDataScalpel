...
- 节点：`TDENGINE_TMQ_INPUT`，从 Canvas `2.1` 引入，仅支持 `STREAMING`；当前 Canvas 为 `2.2`。
...
子查询或 `WITH META`；不提供 Topic 创建、删除、预览、Consumer Group 和手动 Offset 管理。
...
`definitionFingerprint` 为规范化 Topic 类型、数据库、超级表、定义、服务端 Topic ID 和创建时间的
SHA-256。来源或定义变化，以及具有 ID/创建时间的同名重建都会产生新指纹。列表展示全部 Topic，只有
`supported=true` 的条目可被 Canvas 选择。

## 4. Canvas 契约与运行准备

Canvas 大版本仍为 2；TMQ 节点从小版本 1 可用，当前 2.0/2.1 定义可读，保存后规范化为 2.2。节点配置固定为：
...
Topic 超级表快照，Origin 为 `TDENGINE_TMQ`；不自动指定 Event Time 或 Watermark。Compiler 使用同一个
`TdEngineTmqInputNodeOperator` 和 Schema-only I/O 生成零行流 Dataset，不连接 TDengine。
...
配置固定为 WebSocket、`enable.auto.commit=false`、`auto.offset.reset=earliest`、
`MapDeserializer` 和 `msg.with.table.name=true`。内部 Group ID 由数据源 ID、输入节点 ID 和该输出查询的
Checkpoint Location 哈希生成；因此当前“一条 Output 一条 StreamingQuery”的每条 Sink 都获得完整
独立数据流。Client ID 仅用于诊断并带随机 attempt 后缀。
...
最大 Offset 跨度。节点卡片展示数据源、Topic、超级表、EARLIEST/LATEST 和字段数。
...
重新保存发布任务，以新 definitionVersion 和 Checkpoint 前缀建立新部署，不提供通用 Checkpoint 删除
接口。
