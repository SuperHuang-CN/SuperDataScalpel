...
> 该批次能力已经保留在新的多文件、多表模型中；本文记录当时的格式和解析决策。数据模型与 REST API 以[文件数据集管理](file-dataset-management.md)为准，本文中的旧单文件 API 和枚举清单不再是当前契约。
...
- 解压流创建失败、CRC 校验失败或尾部损坏：Job 写入 `FAILED` 和可读错误摘要，临时文件与空表立即清理。
