...
DataModelService 保留模型生命周期编排及原入口，数据查询委托 DataModelDataQueryService，物理表检查、DDL 和变更计划/执行委托 DataModelPhysicalChangeService。ModelDefinitionService 集中维护这些实际调用方共用的字段规范化、来源校验及定义读取。外部查询和 DDL 保持在管理数据库事务外，原有模型、指标与本体字段删除保护继续执行。
