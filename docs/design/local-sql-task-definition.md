...
- 输入模型是显式声明的血缘和编辑辅助信息，不通过正则解析 SQL 推断。第一版默认使用者可信，不引入 SQL Parser 依赖，也不把输入模型声明当作细粒度表访问权限。
...
因此 SQL 词法检查需要把定义规范化为“可选 CTE 部分 + 最外层 SELECT 部分”，方言层再生成最终写入 SQL。该检查只跟踪注释、字符串、引用标识符和括号层级，定位最外层查询边界；不构建通用 SQL AST，也不引入第三方 SQL Parser。
...
SQL 词法检查只承担“单条查询”这一明确边界，不负责推断表血缘、字段权限或兼容全部数据库语法。实现为任务域内职责集中的小类，不引入通用 SQL AST、JSqlParser 或新的解析框架。
...
任务 Service 只提供经过校验的结构化参数，不复制八种数据库语法，也不引入新的第三方 SQL 解析器。

### SQL 与模型引用的关系

输入模型列表是用户显式确认的血缘声明和编辑器 Schema 来源。第一版不解析 `FROM`、`JOIN` 来证明 SQL 只引用了这些模型，也不把物理表名替换为模板变量。原因是八种数据库的 CTE、子查询、函数和标识符规则差异较大，在没有明确权限需求时引入完整 SQL Parser 不符合当前工程基线。
