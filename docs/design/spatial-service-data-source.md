...
`SPATIAL_SERVICE_INPUT` 是批处理输入节点，引用数据源 ID、空间资源 ID 和输出表名。任务准备把无凭据的资源 Schema 与加密解密后的 HTTP 运行连接一并写入 Manifest v10；Runner 按 ArcGIS Query 或 WFS GetFeature 拉取 GeoJSON 分页结果，并用 Sedona 转换 Geometry。Geometry 必须是 EPSG、XY 的明确类型，后续空间处理节点可直接消费。
