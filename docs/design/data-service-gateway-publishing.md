...
flowchart LR
    ...
    Admin -->|"发布 / 取消发布 / 停用时撤回"| GatewayAdmin["Gateway Admin API"]
...
stateDiagram-v2
    ...
    ENABLED --> ENABLED: 取消发布，Engine 保持运行
...
- `ENABLED` 才允许取消发布；没有 Binding 时按幂等成功处理；
- `ENABLED` 才允许停用；
- 定义修改只允许在 Engine 已确认移除且不存在网关绑定时进行；
- 删除只允许在非 `ENABLED`、Engine 已确认移除且不存在网关绑定时进行；
- 发布失败不改变 `ENABLED` 状态；
- 取消发布成功只删除所有 Provider Binding，不改变 Engine 部署、服务 revision、订阅和凭证；
...
-> 短事务：
     成功则删除对应 Binding
     失败则保留 Binding 并写 REMOVE_FAILED
-> 服务保持 ENABLED
-> Engine 保持 DEPLOYED
```
...
```text
POST /api/v1/data-services/{id}/actions/unpublish
```

取消发布清理所有历史 Provider Binding，而不是只清理当前配置 Provider，避免切换网关配置后遗留公开入口。操作成功后调用方不能再通过网关访问，但当前不可变 revision 仍在 Engine 中运行，因此服务定义继续只读；需要修改定义时仍须停用。

Consumer、API Key、服务订阅和 Kong Consumer ACL membership 都不属于服务发布对象，取消发布时保留。重新发布同一服务后，原有订阅关系继续生效。

没有 Binding 时取消发布按幂等成功处理。发布失败可能已在网关留下部分对象，因此 `PUBLISH_FAILED` Binding 也允许取消发布并按稳定名称清理。任一 Provider 清理失败时保留 `REMOVE_FAILED`，页面明确提示网关暴露状态不确定，并允许再次取消发布。
...
停用继续兼具安全撤回职责：即使服务已经执行过取消发布，停用仍以幂等方式确认所有历史 Binding 已清理，再移除 Engine。
...
它只处理未成功启用时的 Engine `PENDING` / `FAILED` 状态。网关发布失败通过再次“发布”修复，或通过“取消发布”清理残留网关对象。
...
- 取消发布失败时服务和 Engine 保持启用，成功后允许直接重新发布相同 revision；
- 网关撤回失败时不停止 Engine；
- Engine 停用失败时网关已撤回，服务仍标记 `ENABLED`，再次停用只需继续移除 Engine；
- Provider 配置变更后，新发布写入当前 Provider；取消发布和停用都会按 Binding 中保存的 Provider 清理所有历史对象。
...
- 数据服务“发布”返回 503；
- 数据服务启用仍可使用；
- 取消发布没有历史网关 Binding 时按幂等成功处理；
- 取消发布存在历史 Binding 时需要对应 Provider 适配器可用；
...
7. 取消发布删除所有网关 Binding，但保持服务 `ENABLED`、Engine `DEPLOYED` 和 revision 不变；
8. 取消发布失败保留 `REMOVE_FAILED` 并允许重试，Engine 不被移除；
9. 取消发布后允许直接重新发布同一 revision；
10. 停用先删除网关 Route/Service，再移除 Engine；
11. 网关撤回失败时 Engine 不被移除；
12. 全局路由路径冲突；
13. 前端取消发布与停用按钮、二次确认、访问模式、状态标签、网关 cURL 和错误提示；
14. 受保护发布按 fail-closed 顺序配置 `key-auth + acl`；
15. 存在订阅时禁止将受保护服务改成公开访问。
...
6. 创建订阅后调用返回 200；
7. 撤回订阅后再次返回 403；
8. 取消发布服务，确认 Kong Route、Service 和 DataScalpel Binding 均已删除；
9. 确认 Engine 仍可运行当前 revision，服务仍为 `ENABLED`；
10. 重新发布并确认原订阅继续生效；
11. 停用服务；
12. 确认网关对象和 Engine 部署均已移除；
13. 清理临时 Consumer、Credential、Subscription、DataService 和网关对象。
