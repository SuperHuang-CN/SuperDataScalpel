...
本文区分 4.20 历史快照、4.31 独立结果、4.32 原字段投影、4.76 当前范围收口与后续目标。开发状态以[进度清单](../canvas-spatial-development-progress.md)为准；旧任务不自动切换算法，不以本地收口宣称完整 GA 对齐。

### 当前阅读入口与对齐边界（4.76 汇总）

- **已接入范围**：4.31 独立分析结果见第 7 节，4.32 中央要素投影见第 9 节，4.76 当前实现范围的血缘、规模与页面收口见第 11 节；第 8 节列明未决时间和公式。
...
- 平均/中位/椭圆时间、官方椭圆公式、真实 Enterprise 与生产容量验收仍未完成；第 11 节只收口当前明确承诺范围，不能把本节投影视为完整 GA 对齐。
...
## 11. 4.76 当前实现范围收口（2026-09-13）

- 五种当前分析结果均完成逐表字段血缘验收。分组字段保持 `DIRECT`；中央要素显式投影的原字段和原
  Geometry 保持 `DIRECT`；平均中心、中位中心、标准距离和方向椭圆 Geometry 以实际 Geometry 与权重字段
  为派生来源。五张结果表均为 `FIELD_COMPLETE`，不含 `WRITTEN_UNKNOWN_SOURCE`。
- 20,000 个投影 XY Point 的单组样例在 Preview/Analyzer 阶段零 Spark Job；平均中心、中位中心、标准距离
  和方向椭圆均完成真实执行。物理计划不含 `CollectLimit` 或 Driver 侧数据收集，但显式包含 Executor 内
  单组 `collect_list`，继续受 100,000 个有效要素和 100 万顶点限制；中央要素仍由 5,000 个有效要素上限保护。
- Engine `SpatialAnalysisProcessorSparkTest` 114 项与 `CenterGeometryStatisticsTest` 8 项通过；前端中心结果
  配置 7 项通过。真实页面已验证新节点延迟编译、默认独立结果、五种分析、逐项结果表/Geometry/σ、排序、
  删除确认、结果模式确认、隐藏草稿保留、无效草稿应用和问题详情；未保存任务定义。
- 本节只表示当前已经承诺的本地能力形成可执行、可追溯和可操作闭环。平均/中位时间、椭圆 interval、
  ArcGIS 官方加权椭圆公式与退化边界、真实 Enterprise 数值及生产容量仍未对齐；标准距离继续明确为平台扩展。
