...
引入于 Canvas 4.12，仅 BATCH，区域表+被汇总表。4.24 增加显式总量分摊、原值地理加权均值及字段统计；4.25 增加可选主表/关联组表及形状组占比；4.37 增加原值交叠比例加权方差/标准差；4.41 增加规则格网区域。仍未完成真实服务、边界及规模对照，不宣称完整等价于 GA Summarize Within。
...
官方结果、共边/多点边界、浏览器全页面及容量验收继续开放，不将本节作为整节点对齐完成声明。
