...
引入于 Canvas 4.14，仅 BATCH；4.20 历史实现是 Point→LineString 与 gap 拆分子集。后续已增加有序片段、测地线、平面面轨迹和缓冲观测窗口；4.44 接入测地面执行链路（第 17 节），全球域与官方对照仍未完成，不能视为完整 Reconstruct Tracks。
...
4.27 有序片段、受控表达式、连接段归属及单点跳过见第 7 节；4.28 显式测地路径见第 8 节。其余目标仍为待开发项，不能直接用于旧小版本 JSON。
...
- **官方参照与差异**：参照 GA Reconstruct Tracks；局部测地面已接入，但全球域、官方公式/精度及容量仍待完成；表达式不承诺 Arcade 兼容。
