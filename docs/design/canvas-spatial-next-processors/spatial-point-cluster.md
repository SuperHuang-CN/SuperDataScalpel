...
## 16. 当前能力收口（2026-09-13，协议仍为 4.76）

- DBSCAN 的旧 Sedona 路径、显式空间密度连通和 Linear 时空密度连通均保留；距离与时间阈值包含边界，
  NULL/Empty Point、NULL 时间、唯一 ID、边界点不合并两个核心簇及日期线测地距离已有执行证据。
- HDBSCAN 的精确核心距离恢复、逐轮割候选、分布式 Borůvka MST、压缩树、EOM、四类诊断和
  Checkpoint 归属/清理形成完整运行闭环。生产路径只向 Driver 返回循环所需的标量计数，
  不收集点、边、簇或成员列表；测试中的小图收集只用于独立真值对照。
- DBSCAN/HDBSCAN 的原字段均保持直接血缘。DBSCAN 的簇号/噪声依赖 Geometry、唯一 ID 和启用的时间字段；
  HDBSCAN 的簇号、噪声及四诊断依赖 Geometry 与唯一 ID，不把隐藏的 DBSCAN 时间草稿误记为来源。
  两种算法的输出均达到 `FIELD_COMPLETE`，没有未知来源字段。
- 20,000 个稀疏平面点的显式 DBSCAN 在本地完成执行，全部行按预期成为噪声；同规模 Preview
  仅分析计划且提交零 Spark Job。HDBSCAN 已有 256 点规则格网的精确候选缩减证据，未用不现实的
  20,000 点密集完整执行冒充容量验收。
- Engine 点聚类/HDBSCAN 联合专项 54 项通过，前端 DBSCAN/HDBSCAN 2 文件 15 项通过。
  真实页面已验证新增节点不立即编译、空间/Linear 条件联动、算法切换确认、HDBSCAN 隐藏半径和时间配置、
  四诊断字段弹窗及无效草稿可继续编辑；未保存任务定义。
- 当前收口不宣称密集候选、极大 `minimumFeatures`、深树 Shuffle/Checkpoint、官方四诊断数值、
  Enterprise 簇成员或生产容量完全等价。`MULTI_SCALE` 仍只是旧草稿占位，不属于 GA 对齐能力。
