...
@JsonClassDescription("支持 BATCH 和 STREAMING 的 Canvas 模型输出节点；把一个或多个逻辑表经显式字段映射写入已发布模型。实时写入采用 micro-batch 至少一次交付，禁止 OVERWRITE 和 Geometry；UPSERT Key 由模型完整主键决定。")
