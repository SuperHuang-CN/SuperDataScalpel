...
@JsonClassDescription("Canvas 编译阶段静态提取的资产级和字段级血缘证据；状态与覆盖度说明证据是否完整可靠。")
public record CanvasLineageCompilation(
        @JsonPropertyDescription("血缘静态分析状态：COMPLETE 完成，PARTIAL 部分完成，UNAVAILABLE 无法形成可靠证据。")
    ...
    @JsonClassDescription("围绕一个 Canvas 输出节点形成的独立数据流及其输入资产、输出资产和字段关系。")
    public record Flow(
            @JsonPropertyDescription("当前编译结果内稳定标识一个输出数据流的键，不是数据库 UUID。")
            String flowKey,
            @JsonPropertyDescription("产生并写出该数据流的 Canvas 输出节点 ID。")
            String outputNodeId,
            @JsonPropertyDescription("产生并写出该数据流的 Canvas 输出节点类型。")
    ...
    @JsonClassDescription("血缘流中的输入或输出资产，可表示纳管模型、JDBC 表或外部资源。")
    public record Asset(
            @JsonPropertyDescription("本次编译结果内稳定引用该资产的键；只保证在当前证据中一致，不是数据库 UUID。")
            ...
            UUID modelId,
            @JsonPropertyDescription("MODEL 资产在本次编译中固定的模型 Schema 版本；其他资产类型为空。")
            Integer modelSchemaVersion,
            @JsonPropertyDescription("JDBC_TABLE 或 EXTERNAL_RESOURCE 所属数据源 UUID；MODEL 资产或无法解析时为空。")
            UUID dataSourceId,
            ...
            @JsonPropertyDescription("HTTP API、空间资源或文件数据集表等已纳管外部资源 UUID；不适用时为空。")
            UUID resourceId,
            @JsonPropertyDescription("外部资源稳定键的 SHA-256，用于关联且不暴露原始敏感标识。")
            String resourceKeyHash,
            @JsonPropertyDescription("经过裁剪和脱敏的资产展示名称；不可作为稳定标识。")
            String safeDisplayName
    ) {
    }

    @JsonClassDescription("血缘快照中的一个输入或输出字段，以及任务对输出字段产生的处理效果。")
    public record Field(
    ...
    @JsonClassDescription("使用当前血缘快照本地键指向一个确定字段。")
    ...
    @JsonClassDescription("一个来源字段到目标字段的直接、计算或聚合派生关系。")
    ...
    @JsonClassDescription("字段在连接、过滤、分组、排序或分区计算中的非输出用途。")
    ...
    @JsonClassDescription("不会阻止编译、但会降低血缘证据覆盖度或可靠性的结构化告警。")
    public record Warning(
            @JsonPropertyDescription("稳定血缘告警码，用于程序识别降级或证据缺失原因。")
            String code,
            @JsonPropertyDescription("血缘分析降级、缺失或无法确认的可读原因。")
            String message,
            @JsonPropertyDescription("告警关联的 Canvas 节点 ID；画布级告警为空。")
            String nodeId,
