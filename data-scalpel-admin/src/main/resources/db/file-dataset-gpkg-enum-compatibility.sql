-- GPKG adds values to Java enums persisted as varchar columns.
-- Hibernate ddl-auto=update does not replace pre-existing enum CHECK constraints.
-- Run once after deploying GPKG-capable binaries and before creating GeoPackage datasets.
...
ALTER TABLE ds_file_dataset
    ...
    CHECK (type IN (
        'CSV', 'TSV', 'TXT', 'JSON', 'JSONL', 'GEOJSON', 'GEOJSONL', 'GEOPARQUET', 'GPKG',
