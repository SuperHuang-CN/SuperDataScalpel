...
@Schema(description = "模型删除和字段变更前使用的当前保护性引用汇总。它聚合已保存任务定义、当前未退役血缘、数据服务定义和已发布指标引用，不包含历史运行快照。")
public record DataModelReferencesResponse(
        @Schema(description = "模型 UUID") UUID modelId,
        @Schema(description = "当前是否完全没有任务、服务或已发布指标保护性引用；不检查模型生命周期。PUBLISHED 模型即使为 true 仍须先停用才能删除。") boolean deletable,
