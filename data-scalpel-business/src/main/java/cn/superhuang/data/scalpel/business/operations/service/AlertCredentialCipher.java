...
@Component
public class AlertCredentialCipher {

    private static final String PREFIX = "v1:";
    private static final byte[] AAD = "datascalpel-alert-channel-v1".getBytes(StandardCharsets.UTF_8);
    ...
    public AlertCredentialCipher(OperationsProperties properties) {
        this.encodedKey = properties.credentialKey();
    }

    public String encrypt(String plaintext) {
        if (plaintext == null || plaintext.isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Webhook 凭据不能为空");
        }
        try {
            ...
            throw unavailable("无法加密 Webhook 凭据", exception);
        }
    }
    ...
    public String decrypt(String ciphertext) {
        try {
            ...
            throw unavailable("无法解密 Webhook 凭据", exception);
        }
    }
    ...
    private static ResponseStatusException unavailable(String message, Exception cause) {
        return new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, message + "，请检查告警凭据密钥配置", cause);
    }
}
