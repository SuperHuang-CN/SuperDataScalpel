...
public class SystemMcpProtocolService {
    ...
    public SystemMcpProtocolService(ObjectMapper mapper,SystemMcpToolService tools,SystemMcpSchemaService schemas,SystemMcpAuditService audits,SystemMcpProperties p) {
        ...
        "api_search","{\"type\":\"object\",\"properties\":{\"query\":{\"type\":\"string\",\"description\":\"检索词；对接口中文名称、模块、关键词、说明和路径执行确定性匹配。\"},\"module\":{\"type\":\"string\",\"description\":\"可选的完整模块名称；用于把结果限制在一个业务模块。\"},\"effect\":{\"type\":\"string\",\"description\":\"可选的操作性质筛选：READ 只读、WRITE 修改数据、EXECUTE 执行业务动作。\",\"enum\":[\"READ\",\"WRITE\",\"EXECUTE\"]},\"offset\":{\"type\":\"integer\",\"description\":\"结果偏移量，从 0 开始；默认 0。\",\"minimum\":0},\"limit\":{\"type\":\"integer\",\"description\":\"本次返回数量；默认 10，最多 50。\",\"minimum\":1,\"maximum\":50}},\"additionalProperties\":false}",
        "api_describe","{\"type\":\"object\",\"properties\":{\"operationIds\":{\"type\":\"array\",\"description\":\"需要读取最新契约的稳定接口标识列表；标识来自 api_search，一次 1 至 5 个。\",\"items\":{\"type\":\"string\"},\"minItems\":1,\"maxItems\":5}},\"required\":[\"operationIds\"],\"additionalProperties\":false}",
        "api_invoke","{\"type\":\"object\",\"properties\":{\"operationId\":{\"type\":\"string\",\"description\":\"要执行的稳定接口标识；执行前应通过 api_describe 获取最新契约。\"},\"pathParams\":{\"type\":\"object\",\"description\":\"路径参数对象；字段和值必须符合 api_describe 返回的 inputSchema。\"},\"queryParams\":{\"type\":\"object\",\"description\":\"查询参数对象；字段和值必须符合 api_describe 返回的 inputSchema。\"},\"body\":{\"description\":\"JSON 请求体；对象、数组或标量形态必须符合 api_describe 返回的 inputSchema。\"}},\"required\":[\"operationId\"],\"additionalProperties\":false}");
