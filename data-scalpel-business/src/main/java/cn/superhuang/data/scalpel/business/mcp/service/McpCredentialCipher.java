...
public class McpCredentialCipher {
    ...
    private static final byte[] AAD = "datascalpel-mcp-access-token-v1".getBytes(StandardCharsets.UTF_8);
    private static final int IV_BYTES = 12;
    private static final int TAG_BITS = 128;
    private final String encodedKey;
    private final SecureRandom secureRandom = new SecureRandom();

    public McpCredentialCipher(McpPlatformProperties properties) {
        encodedKey = properties.credentialKey();
    }

    public String encrypt(String plaintext) {
        if (plaintext == null || plaintext.isBlank()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "MCP Access Token 不能为空");
        try {
            ...
            throw unavailable("无法加密 MCP Access Token", exception);
        }
    }

    public String decrypt(String ciphertext) {
        try {
            if (ciphertext == null || !ciphertext.startsWith(PREFIX)) throw new GeneralSecurityException("unsupported ciphertext version");
            byte[] payload = Base64.getDecoder().decode(ciphertext.substring(PREFIX.length()));
            if (payload.length <= IV_BYTES) throw new GeneralSecurityException("invalid ciphertext");
            ...
            throw unavailable("无法解密 MCP Access Token", exception);
        }
    }
    ...
    private Cipher cipher(int mode, byte[] iv) throws GeneralSecurityException {
        byte[] key;
        try {
            if (encodedKey == null || encodedKey.isBlank()) throw new GeneralSecurityException("credential key is missing");
    ...
    private static ResponseStatusException unavailable(String message, Exception cause) {
        return new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,
                message + "，请检查 data-scalpel.mcp.credential-key 配置", cause);
    }
}
