...
@Component
public class ServiceEngineCredentialCipher {

    private static final String PREFIX = "v1:";
    private static final byte[] AAD =
            "datascalpel-service-engine-management-token-v1".getBytes(StandardCharsets.UTF_8);
    ...
    public ServiceEngineCredentialCipher(ServiceEngineSecurityProperties properties) {
    ...
    private Cipher cipher(int mode, byte[] iv) throws GeneralSecurityException {
        ...
        try {
            if (encodedKey == null || encodedKey.isBlank()) {
                throw new GeneralSecurityException("credential key is missing");
            }
    ...
    private static ResponseStatusException unavailable(String message, Exception cause) {
        return new ResponseStatusException(
                ...
                message + "，请检查 Service Engine 凭据加密密钥配置",
                cause
        );
    }
}
