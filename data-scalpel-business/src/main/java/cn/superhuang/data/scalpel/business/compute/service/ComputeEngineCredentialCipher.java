...
@Component
public class ComputeEngineCredentialCipher {

    private static final String PREFIX = "v1:";
    private static final byte[] AAD = "datascalpel-compute-engine-token-v1".getBytes(StandardCharsets.UTF_8);
    ...
    public ComputeEngineCredentialCipher(ComputeEngineProperties properties) {
        this.encodedKey = properties.credentialKey();
    }

    public String encrypt(String plaintext) {
        if (plaintext == null || plaintext.isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Dispatcher 访问令牌不能为空");
        }
        try {
            ...
            throw unavailable("无法加密 Dispatcher 访问令牌", exception);
        }
    }
    ...
    public String decrypt(String ciphertext) {
        try {
            ...
            throw unavailable("无法解密 Dispatcher 访问令牌", exception);
        }
    }
    ...
    private static ResponseStatusException unavailable(String message, Exception cause) {
        return new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, message + "，请检查计算引擎凭据密钥配置", cause);
    }
}
