...
@Component
public class AssistantCredentialCipher {

    private static final String PREFIX = "v1:";
    private static final byte[] AAD = "datascalpel-assistant-credentials-v1".getBytes(StandardCharsets.UTF_8);
    ...
    public AssistantCredentialCipher(AssistantProperties properties) {
    ...
    public String encryptOptional(String plaintext) {
        ...
        try {
            ...
            byte[] encrypted = cipher(Cipher.ENCRYPT_MODE, iv)
                    .doFinal(plaintext.trim().getBytes(StandardCharsets.UTF_8));
            byte[] payload = new byte[iv.length + encrypted.length];
            System.arraycopy(iv, 0, payload, 0, iv.length);
            System.arraycopy(encrypted, 0, payload, iv.length, encrypted.length);
            return PREFIX + Base64.getEncoder().encodeToString(payload);
        } catch (GeneralSecurityException exception) {
            throw unavailable("无法加密 AI 模型凭据", exception);
        }
    }
    ...
    public String decryptOptional(String ciphertext) {
        ...
        try {
            if (!ciphertext.startsWith(PREFIX)) throw new GeneralSecurityException("unsupported ciphertext version");
            ...
            throw unavailable("无法解密 AI 模型凭据", exception);
        }
    }
    ...
    private Cipher cipher(int mode, byte[] iv) throws GeneralSecurityException {
        byte[] key;
        try {
            key = encodedKey == null || encodedKey.isBlank()
    ...
    private static ResponseStatusException unavailable(String message, Exception cause) {
        return new ResponseStatusException(
                ...
                message + "，请检查 data-scalpel.assistant.credential-key 配置",
                cause
        );
    }
}
