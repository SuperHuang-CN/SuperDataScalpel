...
export const DataEntryInputField = ({ formId, field }: { formId: string; field: DataEntryField }) => {
  ...
  if (hasOptions) {
    ...
    control = <Select allowClear={field.nullable} options={[{ value: true, label: '是' }, { value: false, label: '否' }]} />;
  } else if (['BYTE', 'SHORT', 'INTEGER'].includes(field.fieldType)) {
    control = <InputNumber precision={0} style={{ width: '100%' }} />;
    ...
    control = <DatePicker showTime style={{ width: '100%' }} />;
  } else if (field.fieldType === 'STRING' && (!field.length || field.length > 500)) {
    control = <Input.TextArea rows={4} maxLength={field.length ?? undefined} />;
  } else {
    control = <Input maxLength={field.length ?? undefined} />;
  }
