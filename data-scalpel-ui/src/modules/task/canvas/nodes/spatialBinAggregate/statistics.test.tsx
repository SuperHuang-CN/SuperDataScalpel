...
describe('bin field statistics', () => {
  ...
  it('cancels isolated edits and permits saving invalid business drafts', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '设置格网统计项' }));
    fireEvent.change(screen.getByRole('textbox', { name: '统计输出字段 1' }), { target: { value: 'discard' } });
    ...
    await userEvent.click(screen.getByRole('button', { name: '设置格网统计项' }));
    fireEvent.change(screen.getByRole('textbox', { name: '统计输出字段 1' }), { target: { value: '' } });
    await userEvent.click(screen.getByRole('button', { name: '添加统计' }));
    await userEvent.click(screen.getByRole('combobox', { name: '统计类型 2' }));
    await userEvent.click(screen.getByText('ANY · 字符串样本'));
    fireEvent.click(screen.getByRole('button', { name: '保存统计草稿' }));
  ...
  it('restores a field when toggling Count and preserves unavailable fields through sorting', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '设置格网统计项' }));
    await userEvent.click(screen.getByRole('combobox', { name: '统计类型 2' }));
    await userEvent.click(screen.getAllByText('COUNT · 点数').at(-1)!);
    await userEvent.click(screen.getByRole('combobox', { name: '统计类型 2' }));
    await userEvent.click(screen.getByText('COUNT_FIELD · 非空数'));
    fireEvent.click(screen.getByRole('button', { name: '上移统计 2' }));
    fireEvent.click(screen.getByRole('button', { name: '保存统计草稿' }));
