...
const body = ({ data }: CanvasNodeBodyProps<typeof CanvasNodeType.Union>) => {
  ...
  return <NodeContent variant="dual">
    ...
    <NodeBadges><NodeBadge tone="strong">{mode ?? '待设置模式'}</NodeBadge><NodeBadge>{inputTableNames.length} 张表</NodeBadge><NodeBadge>{fieldCountText(result)}</NodeBadge></NodeBadges>
  </NodeContent>;
};
