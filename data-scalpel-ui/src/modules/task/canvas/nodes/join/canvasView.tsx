...
const body = ({ data }: CanvasNodeBodyProps<typeof CanvasNodeType.Join>) => {
  ...
  return <NodeContent variant="dual">
    ...
    <NodeBadges><NodeBadge tone="strong">{joinType ?? '待设置类型'}</NodeBadge><NodeBadge>{conditions.length} 个条件</NodeBadge><NodeBadge>{fieldCountText(result)}</NodeBadge></NodeBadges>
  </NodeContent>;
};
