...
import { inputTable, outputTable, resolvedNodeSize } from '../canvasNodePresentation';
import type { CanvasNodeBodyProps, CanvasNodeCanvasView } from '../nodeSpec';

const body = ({ data }: CanvasNodeBodyProps<typeof CanvasNodeType.StreamJoin>) => {
  const { leftTableName, rightTableName, outputTableName, joinType, conditions } = data.configuration;
  ...
  return <NodeContent variant="dual">
    ...
    <NodeBadges><NodeBadge tone="info">STREAM + LOOKUP</NodeBadge><NodeBadge>{stream?.eventTimeColumn ? `事件时间 ${stream.eventTimeColumn}` : '等待事件时间'}</NodeBadge>{result?.watermarkDelay && <NodeBadge>WM {result.watermarkDelay}</NodeBadge>}</NodeBadges>
  </NodeContent>;
};
